...
// WebGL2 fragment-shader hero background ("Tron"-style energy field, blue
// palette). Adapted from a TS/Next.js component: styled-jsx doesn't exist
// here (plain Vite/React) so the keyframes moved to index.css, and
// touch-action is pan-y instead of none - a full-viewport canvas with
// touch-none would swallow the vertical swipe and re-lock mobile scroll.
...
* adapted to a muted blue "energy field" palette for BILON
*/
precision highp float;
out vec4 O;
...
function useShaderBackground() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      return; // no WebGL2 - the CSS gradient fallback behind the canvas stays visible
    }
...
export default function ShaderHero({ trustBadge, headline, subtitle, buttons, className = '' }) {
  ...
  return (
    <div className={`relative w-full min-h-dvh overflow-hidden bg-[#050b16] ${className}`}>
      ...
      <div className="relative z-10 flex min-h-dvh flex-col items-center justify-center px-6 py-24 text-white">
        {trustBadge && (
          <div className="lp-hero-fade-down mb-8">
            <div className="flex items-center gap-2 rounded-full border border-sky-300/25 bg-sky-950/60 px-6 py-3 text-sm">
